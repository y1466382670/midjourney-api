...
public class UserWebSocketStarter extends WebSocketAdapter implements WebSocketStarter {
	...
	private final MyProxyConfig proxyConfig;
	...
	public UserWebSocketStarter(String wssServer, DiscordAccount account, UserMessageListener userMessageListener, MyProxyConfig proxyConfig) {
	...
	private void send(int op, Object d) {
		if (this.socket != null) {
			log.debug("wss-send-socket" + DataObject.empty().put("op", op).put("d", d));
	...
	private DataObject createAuthData() {
		...
		DataObject connectionProperties = DataObject.empty()
				...
				.put("client_build_number", 222963) //251616
		...
		return DataObject.empty()
				...
				.put("token", this.account.getToken());
	}
}

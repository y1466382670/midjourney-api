...
public class DiscordServiceImpl implements DiscordService {
	private static final String DEFAULT_SESSION_ID = "cabc2064-fcc8-4eaa-80a0-88047315e852";
	...
	@Override
	public Message<Void> action(String messageId, String messageHash, String actionCommand, String nonce) {
		String paramsStr = replaceInteractionParams(this.paramsMap.get("action"), nonce)
				.replace("$message_id", messageId)
				.replace("$action", actionCommand)
				.replace("$message_hash", messageHash);
		return postJsonAndCheckStatus(paramsStr);
	...
	public Message<Void> zoomout(String messageId, String messageHash, int number, String nonce) {
		String paramsStr = replaceInteractionParams(this.paramsMap.get("zoom_out"), nonce)
				.replace("$message_id", messageId)
				.replace("$message_hash", messageHash)
	...
	private String replaceInteractionParams(String paramsStr, String nonce) {
		return paramsStr
				...
				.replace("$session_id", System.currentTimeMillis() + RandomUtil.randomNumbers(3))
